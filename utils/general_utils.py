...
def _get_custom_exp_code(args):
    ...
    param_code += "_pel" + str(args.use_pel)
    param_code += "_mel" + str(args.use_mel)
    param_code += "_cgf" + str(args.use_cgf)
    param_code += "_rw" + str(args.ratio_wsi)
    param_code += "_sg" + str(args.sigma)
    param_code += "_th" + str(args.theta)
    param_code += "_cm" + str(args.comp_margin)
...
def _get_split_loader(args, split_dataset, training = False, testing = False, weighted = False, batch_size=1):
    ...
    } if args.device.type == "cuda" else {'num_workers': args.num_workers}
