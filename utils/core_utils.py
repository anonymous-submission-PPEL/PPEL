import numpy as np
import os
from models.model_PPEL import PPEL
...
try:
    ...
    def concordance_index_censored(event_indicator, event_time, estimate, tied_tol=1e-08):
        ...
        concordant, permissible, tied = 0.0, 0.0, 0.0
        n = len(event_time)
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if event_time[i] < event_time[j]:
                    ...
                    early, late = j, i
                if not event_indicator[early]:
                    continue
                permissible += 1
                diff = estimate[early] - estimate[late]
                if abs(diff) <= tied_tol:
                    tied += 1
                elif diff > 0:
                    concordant += 1
        c_index = (concordant + 0.5 * tied) / permissible if permissible > 0 else 0.0
        return c_index, concordant, permissible, tied, 0

    def concordance_index_ipcw(*args, **kwargs):
        raise ImportError("sksurv is required for IPCW C-index.")

    def brier_score(*args, **kwargs):
        raise ImportError("sksurv is required for Brier score.")

    def integrated_brier_score(*args, **kwargs):
        raise ImportError("sksurv is required for integrated Brier score.")

    def cumulative_dynamic_auc(*args, **kwargs):
        raise ImportError("sksurv is required for dynamic AUC.")

    class Surv:
        @staticmethod
        def from_arrays(event, time):
            return {"event": np.asarray(event).astype(bool), "time": np.asarray(time)}
...
def _init_optim(args, model):
    ...
    if args.opt == "adam":
        ...
        if RAdam is None:
            raise ImportError("RAdam is unavailable. Use --opt adam/adamW or install custom_optims.")
        optimizer = RAdam(model.parameters(), lr=args.lr, weight_decay=args.reg)
    elif args.opt == "lamb":
        raise NotImplementedError("LAMB optimizer is not bundled with this project.")
    else:
        raise NotImplementedError
...
def _train_loop_survival(args, epoch, model, omics_format, loader, optimizer, loss_fn, log_file):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        logits, comp_loss, evidence_loss = h[0], h[1], h[2]

        loss_surv = loss_fn(h=logits, y=y_disc, t=event_time, c=censor)

        loss = loss_surv + args.sigma * comp_loss + args.theta * evidence_loss
        
        if torch.isnan(loss) or torch.isinf(loss):
            print(f"⚠️ NaN/Inf in total loss at batch {batch_idx}. Skipping this batch.")
            log_file.write(f"NaN in total loss at batch {batch_idx}\n")
            continue
...
def _summary(dataset_factory, model, omics_format, loader, loss_fn, survival_train=None):
    ...
    # print("{")
    # print(f'    "event_times": [{", ".join(map(str, all_event_times))}],')
    # print(f'    "censorships": [{", ".join(map(str, all_censorships))}],')
    # print(f'    "risk_scores": [{", ".join(map(str, all_risk_scores))}]')
    # print("},")
    patient_results = {}
