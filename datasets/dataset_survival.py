...
class SurvivalDatasetFactory:
    ...
    def return_splits(self, args, csv_path, fold):
        ...
        if args.only_test:
            val_split.drop_omics = True
            print("⚠️ Test mode: genomics modality dropped!")
    ...
    def _get_split_from_df(self, args, all_splits, split_key: str = 'train', fold=None, scaler=None, valid_cols=None):
        ...
        # mask = self.label_data['slide_id'].isin(split.tolist())
...
class SurvivalDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.sample = sample
        self.mode = mode
        self.drop_omics = False   # default: no dropping
    ...
    def __getitem__(self, idx):
        ...
        if self.drop_omics:   # <- test 阶段你会设为 True
            patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)

            if self.omics_format == "groups":
                # six omics → use zero vectors with correct dims
                fake1 = torch.zeros_like(torch.tensor(self.omics_data_dict["rna"][self.omic_names[0]].iloc[0]))
                fake2 = torch.zeros_like(torch.tensor(self.omics_data_dict["rna"][self.omic_names[1]].iloc[0]))
                fake3 = torch.zeros_like(torch.tensor(self.omics_data_dict["rna"][self.omic_names[2]].iloc[0]))
                fake4 = torch.zeros_like(torch.tensor(self.omics_data_dict["rna"][self.omic_names[3]].iloc[0]))
                fake5 = torch.zeros_like(torch.tensor(self.omics_data_dict["rna"][self.omic_names[4]].iloc[0]))
                fake6 = torch.zeros_like(torch.tensor(self.omics_data_dict["rna"][self.omic_names[5]].iloc[0]))

                return (
                    patch_features, fake1, fake2, fake3, fake4, fake5, fake6,
                    label, event_time, c, clinical_data, mask
                )

            elif self.omics_format == "pathways":
                # omics_list: list of N pathway vectors
                fake_list = []
                for i in range(self.num_pathways):
                    fake = torch.zeros_like(
                        torch.tensor(self.omics_data_dict["rna"][self.omic_names[i]].iloc[0])
                    )
                    fake_list.append(fake)

                return (
                    patch_features, fake_list,
                    label, event_time, c, clinical_data, mask
                )

            elif self.omics_format == "gene":
                df_small = self.omics_data_dict["rna"].drop(columns="temp_index")
                fake_vec = torch.zeros(df_small.shape[1])   # correct gene dimension
                return (torch.zeros((1, 1)), fake_vec, label, event_time, c, clinical_data)

            else:
                raise NotImplementedError

        if self.omics_format == "gene":
            ...
            return (
                patch_features, omic1, omic2, omic3, omic4, omic5, omic6,
                label, event_time, c, clinical_data, mask
            )
            ...
            for i in range(self.num_pathways):
                omic_list.append(
                    torch.tensor(self.omics_data_dict["rna"][self.omic_names[i]].iloc[idx])
                )
    ...
    def _load_wsi_embs_from_path(self, data_dir, slide_ids, case_id=None):
        """
        Optimized version:
        - Force CPU loading (map_location='cpu')
        - Concatenate & sample on CPU
        - Transfer to GPU only ONCE (outside Dataset)
        """
        ...
        for slide_id in slide_ids:
            ...
            base_name = os.path.splitext(slide_id)[0]
            pt_path = os.path.join(data_dir, f"{base_name}.pt")
            h5_path = os.path.join(data_dir, f"{base_name}.h5")

            if os.path.isfile(pt_path):
                wsi_bag = torch.load(pt_path, map_location='cpu')
            elif os.path.isfile(h5_path):
                try:
                    import h5py
                except ImportError as exc:
                    raise ImportError("h5py is required to load WSI .h5 feature files") from exc

                with h5py.File(h5_path, "r") as f:
                    if "features" not in f:
                        raise KeyError(f"{h5_path} does not contain a 'features' dataset")
                    wsi_bag = torch.from_numpy(f["features"][:]).float()
            else:
                raise FileNotFoundError(f"Cannot find WSI features for {slide_id}: {pt_path} or {h5_path}")

            patch_list.append(wsi_bag)

        patch_features = torch.cat(patch_list, dim=0)  # still in CPU

        if self.mode == "cluster":
            cluster_ids = np.load(
                os.path.join(data_dir, 'patch-l1-cluster10-ids', f"{case_id}.npy")
            )
            mask = torch.from_numpy(cluster_ids).float()
        else:
            mask = None
        ...
        if self.sample:
            ...
            n_samples = min(total_patches, max_patches)
            idx = np.sort(np.random.choice(total_patches, n_samples, replace=False))

            patch_features = patch_features[idx, :]  # CPU slicing

            if n_samples < max_patches:
                pad_len = max_patches - n_samples
                pad = torch.zeros([pad_len, patch_features.shape[1]], dtype=patch_features.dtype)
                patch_features = torch.cat([patch_features, pad], dim=0)

                if mask is not None:  # cluster mode
                    mask = torch.cat([mask[idx], torch.zeros(pad_len)])
            else:
                if mask is not None:
                    mask = mask[idx]

            if mask is None:
                mask = torch.zeros([patch_features.shape[0]])  # 0 = real patch, 1 = padded
        else:
            # no sampling
            if mask is None:
                mask = torch.ones([1])
